...
def cli_collect_ais(
    ...
    cfg = load_config(config)
    out = collect_ais_for_schedule(cfg, Path(schedule_csv), window_min=window_min, out_dir=Path(out_dir) if out_dir else None, dry_run=dry_run)
...
def cli_run(
    ...
    run_end_to_end(cfg_path=config, dry_run_ais=dry_run_ais, max_chips=max_chips)
